...
inline Uint16* tile_bank_mem(Uint16 i){
  return (&gfx_mem_vram[((TILE_BANK_SIZE/2)*(i))]);
}

inline Tile4* tile_bank_mem_4bpp(Uint16 i){
  return ((Tile4*)tile_bank_mem(i));
}

inline Tile8* tile_bank_mem_8bpp(Uint16 i){
  return ((Tile8*)tile_bank_mem(i));
}
